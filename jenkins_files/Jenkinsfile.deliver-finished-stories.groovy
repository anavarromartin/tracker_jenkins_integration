...
def deliverFinishedStories(projectId) {
    stories = jsonParse(doGetHttpRequest(HOST + '/services/v5/projects/' + projectId + '/stories?with_state=finished').body)

    for (int i = 0; i < stories.size(); i++) {
        println "Story ${stories[i].id}"
        doPutHttpRequestWithJson("{\"current_state\": \"delivered\"}",
